...
parser.add_argument('--jet-idx', choices=['1', '2', 'x'], help='Jet index used for training/predicting.')
parser.add_argument('-i', '--inputdir', help='Input dir')
parser.add_argument('-o', '--outputdir', help='Output dir')
parser.add_argument('-s', '--sigexpr', help='sigexpr')
parser.add_argument('-b', '--bkgexpr', help='bkgexpr')
...
# preprocessing
basedirs = [
    '/home/pku/sdeng/sfbdt/test_output/', # sample dedicated for training
    # '/data/pku/home/licq/hcc/new/samples/trees_sf/20221023_flvEnrichedQCD_v2_ULNanoV9_ak8_qcd_2017/', # sample dedicated for training
]
flist_train = [
    ('qcd-ccc', 'mc/qcd-ccc-enriched_tree.root'),
    ('qcd-bbb', 'mc/qcd-bbb-enriched_tree.root'),
]
flist_pred = [
    # 'mc/qcd-mg_tree.root',
    'fullAOD_QCD_bbb.root',
    # 'mc/zhbb_tree.root',
    # 'mc/zhcc_tree.root',
    # 'mc/ggH-cc_tree.root',
    # 'mc/qcd-ccc-enriched_tree.root',
    # 'mc/qcd-bbb-enriched_tree.root',
...
basesel = f'passHTTrig & fj_{x}_pt>200 & fj_{x}_sdmass>50 & fj_{x}_sdmass<200 & passmetfilters & (fj_{x}_nbhadrons>0 | fj_{x}_nchadrons>0) & fj_{x}_is_qualified'
...
sigexpr = args.sigexpr
bkgexpr = args.bkgexpr

# sigexpr = f'fj_{x}_gpart_ptratio < 0.10'; bkgexpr = f'fj_{x}_gpart_ptratio > 0.75'

# sigexpr = f'(fj_{x}_npart <= 2 & (fj_{x}_bpart_ptratio > 0.90 | fj_{x}_cpart_ptratio > 0.90)) | (fj_{x}_npart > 2 & fj_{x}_gpart_ptratio < 0.3)'
# bkgexpr = f'(fj_{x}_npart <= 2 & (fj_{x}_bpart_ptratio < 0.10 & fj_{x}_cpart_ptratio < 0.10)) | (fj_{x}_npart > 2 & fj_{x}_gpart_ptratio > 0.8)'

# sigexpr = f'(fj_{x}_npart <= 2 & (fj_{x}_bpart_ptratio > 0.80 | fj_{x}_cpart_ptratio > 0.80)) | (fj_{x}_npart > 2 & (fj_{x}_gpart_ptratio < 0.40 & fj_{x}_gpart_ptratio > 0.20))'
# bkgexpr = f'(fj_{x}_npart <= 2 & (fj_{x}_bpart_ptratio < 0.20 & fj_{x}_cpart_ptratio < 0.20)) | (fj_{x}_npart > 2 & fj_{x}_gpart_ptratio > 0.80)'

# sigexpr = f'(fj_{x}_npart <= 2 & (fj_{x}_bpart_ptratio > 0.80 | fj_{x}_cpart_ptratio > 0.80)) | (fj_{x}_npart > 2 & (fj_{x}_bpart_ptratio > 0.70 | fj_{x}_cpart_ptratio > 0.70))'
# bkgexpr = f'(fj_{x}_npart <= 2 & (fj_{x}_bpart_ptratio < 0.30 & fj_{x}_cpart_ptratio < 0.30)) | (fj_{x}_npart > 2 & (fj_{x}_bpart_ptratio < 0.20 & fj_{x}_cpart_ptratio < 0.20))'

# sigexpr = f'(fj_{x}_npart <= 2 & fj_{x}_gpart_ptratio < 0.10) | (fj_{x}_npart > 2 & fj_{x}_gpart_ptratio < 0.15)'
# bkgexpr = f'(fj_{x}_npart <= 2 & fj_{x}_gpart_ptratio > 0.85) | (fj_{x}_npart > 2 & fj_{x}_gpart_ptratio > 0.75)'

# sigexpr = f'fj_{x}_nbpart == 2 | fj_{x}_ncpart == 2'
# bkgexpr = f'fj_{x}_nbpart != 2 & fj_{x}_ncpart != 2'

# sigexpr = f'(fj_{x}_nbpart == 2 & fj_{x}_bpart_ptratio > 0.80) | (fj_{x}_ncpart == 2 & fj_{x}_cpart_ptratio > 0.80)'
# bkgexpr = f'fj_{x}_nbpart != 2 & fj_{x}_ncpart != 2 & fj_{x}_gpart_ptratio > 0.85 '
# bkgexpr = f'fj_{x}_bpart_ptratio < 0.10 & fj_{x}_cpart_ptratio < 0.10'
...
obs_vars = [
    ...
    f'fj_{x}_btagDDBvLV2',
    f'fj_{x}_btagHbb',
    f'fj_{x}_DeepAK8_ZHbbvsQCD',
    f'fj_{x}_DeepAK8MD_ZHbbvsQCD',
    f'fj_{x}_DeepAK8MD_bbVsLight',
    f'fj_{x}_DeepAK8MD_bbVsTop',
    f'fj_{x}_DeepAK8MD_HbbvsQCD',
    f'fj_{x}_ParticleNetMD_XccVsQCD',

    f'fj_{x}_DeepAK8MD_ZHccvsQCD',
    f'fj_{x}_DeepAK8MD_ccVsLight',
    f'fj_{x}_btagDDCvLV2',
    f'fj_{x}_btagDDCvBV2',

    f'fj_{x}_sfBDT',
    f'fj_{x}_matchedInitHadsTau3',
    f'fj_{x}_matchedInitHadsTau2',
    f'fj_{x}_matchedInitHadsTau1',
]
# all_vars = set(train_vars + obs_vars) - set([f'fj_{x}_cpart_ptratio', f'fj_{x}_gpart_ptratio', f'fj_{x}_bpart_ptratio'])
all_vars = set(train_vars + obs_vars) - set([f'fj_{x}_tau31', f'fj_{x}_cpart_ptratio', f'fj_{x}_gpart_ptratio', f'fj_{x}_bpart_ptratio'])
...
def add_vars(df):
    ...
    df[f'fj_{x}_tau31'] = df.eval(f'fj_{x}_tau32*fj_{x}_tau21')
    return df
...
def make_dmatrix(filepath='', predict=False, k_folds=5, random_state=None):
    if predict:
        ...
        df = fix_uint64_branches(uproot.open(filepath)['Events'].pandas.df(all_vars))
        # df = fix_uint64_branches(uproot.open(filepath)['Events'].pandas.df(flatten=False))
        add_vars(df)
        print('...done')
        X = df[train_vars]
        outputs = []
